...
fn validate_checkpoint_turn(
    ...
    for (field, terms) in [
        ("required_answer_terms", &rubric.required_answer_terms),
        ("forbidden_answer_terms", &rubric.forbidden_answer_terms),
    ] {
        validate_terms_not_blank(&format!("{path}.evaluation.{field}"), field, terms, issues);
    }
    ...
    for (expectation_index, expectation) in rubric.expected_memories.iter().enumerate() {
        ...
        for (field, terms) in [
            ("answer_terms", &expectation.answer_terms),
            ("trace_terms", &expectation.trace_terms),
        ] {
            validate_terms_not_blank(&format!("{expectation_path}.{field}"), field, terms, issues);
        }
...
/// Blank terms are scoring poison: they join `fraction_present`'s
/// denominator but can never match, silently capping the term score below
/// 1.0 with no visible finding — so validation rejects them outright.
fn validate_terms_not_blank(
    path: &str,
    field: &str,
    terms: &[String],
    issues: &mut Vec<EvalValidationIssue>,
    ...
    for (index, term) in terms.iter().enumerate() {
        if term.trim().is_empty() {
            issues.push(EvalValidationIssue {
                ...
                message: format!("`{field}` entries must not be blank"),
            });
        }
    }
}
    ...
    D: EvalDriver + ?Sized,
{
    ...
    // to the graph, so one probe pass covers every sample. The probe pass and
    // the metabolism counters are independent reads: fetch them concurrently.
    let ((probes, mut total_usage), graph_stats) =
        tokio::join!(run_memory_probes(driver, &rubric, profile.judge), async {
            driver.graph_stats().await.unwrap_or(None)
        },);
...
{
    // Expectations are independent read-only probes; run them concurrently
    // and aggregate in rubric order (`join_all` preserves it) so reports and
    // findings stay deterministic.
    let outcomes = join_all(
        rubric
            .expected_memories
            .iter()
            .map(|expectation| run_memory_probe(driver, expectation, judge_kind)),
    )
    .await;

    let mut probes = Vec::new();
    let mut usage = Usage::default();
    for (probe, probe_usage) in outcomes.into_iter().flatten() {
        usage.accumulate(&probe_usage);
        probes.push(probe);
    }
    (probes, usage)
}

/// Probes one expectation. `None` when it declares neither a semantic
/// assertion (usable with the LLM judge) nor a raw probe.
async fn run_memory_probe<C>(
    driver: &C,
    expectation: &ExpectedMemory,
    judge_kind: EvalJudgeKind,
) -> Option<(MemoryProbeReport, Usage)>
where
    C: AssessContext + ?Sized,
{
    let mut usage = Usage::default();
    ...
    if let (Some(assertion), EvalJudgeKind::Llm) = (&expectation.assertion, judge_kind) {
        ...
        // the assertion trivially does not hold, so a judge call would be
        // pure spend.
        let (satisfied, judge_reason) = if hit_count == 0 && evidence_is_empty(&evidence) {
            (
                expectation.mode == MemoryExpectationMode::ShouldNotExist,
                Some("no graph evidence matched the probe search".to_string()),
            )
        } else {
    ...
    let request = expectation.probe.clone()?;
...
/// True when a semantic probe search surfaced nothing to judge.
fn evidence_is_empty(evidence: &Json) -> bool {
    match evidence {
        Json::Null => true,
...
mod tests {
    ...
    struct FakeEvalDriver {
        ...
        /// Every `complete` call as (instructions, prompt), for asserting
        /// what the harness showed a judge or simulator.
        seen_completions: Mutex<Vec<(String, String)>>,
    ...
    impl AssessContext for FakeEvalDriver {
        async fn complete(&self, req: CompletionRequest) -> Result<AgentOutput, BoxError> {
            self.seen_completions
                .lock()
                .unwrap()
                .push((req.instructions.clone(), req.prompt.clone()));
                ...
                evaluation: Some(EvalRubric {
                    expected_memories: vec![
                        ...
                        ExpectedMemory {
                            ...
                            assertion: Some("a stale BBQ preference".to_string()),
                            ..Default::default()
                        },
                        ExpectedMemory {
                            id: "missing".to_string(),
                            mode: MemoryExpectationMode::ShouldExist,
                            assertion: Some("a vegetarian preference".to_string()),
                            ..Default::default()
                        },
                    ],
                    ..Default::default()
    ...
    fn validate_eval_plan_reports_offline_input_errors() {
        let scenario = EvalScenario {
            ...
            timeline: vec![
                ...
                EvalTurn {
                    ...
                    evaluation: Some(EvalRubric {
                        required_answer_terms: vec!["direct".to_string(), "  ".to_string()],
        ...
        // Blank terms silently cap `fraction_present` at runtime, so both the
        // rubric-level and expectation-level lists must reject them.
        assert!(report.issues.iter().any(|issue| {
            issue.severity == EvalValidationSeverity::Error
                && issue
                    .message
                    .contains("`required_answer_terms` entries must not be blank")
        }));
        assert!(report.issues.iter().any(|issue| {
            issue.severity == EvalValidationSeverity::Error
                && issue
                    .message
                    .contains("`answer_terms` entries must not be blank")
        }));
        assert!(report.issues.iter().any(|issue| {
    ...
    async fn semantic_assertion_probe_uses_judge_verdict() {
        ...
        // The search surfaces a (superseded) memory, so the judge must run
        // and decide whether it counts as active; empty evidence would be
