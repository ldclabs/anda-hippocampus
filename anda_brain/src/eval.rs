    ...
    D: EvalDriver + ?Sized,
{
    ...
    let probes = run_memory_probes(driver, &rubric, profile.judge).await?;
    ...
    let mut findings = majority_findings(&samples);
                ...
                match judge::judge_assertion(driver, assertion, &evidence).await {
                    Ok(verdict) => {
                        let satisfied = match expectation.mode {
                            MemoryExpectationMode::ShouldExist => verdict.holds,
                            MemoryExpectationMode::ShouldNotExist => !verdict.holds,
                        };
                        (satisfied, Some(verdict.reason))
                    }
                    Err(err) => {
...
fn score_checkpoint(
    ...
    if let Some(verdict) = verdict {
        findings.extend(verdict.findings.iter().cloned());
    }
